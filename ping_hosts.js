...
// pinghosts(backend_settings.minerips);
 ...
 exports.pinghosts = function(hosts){
    ...
    hosts.forEach((host) => {
        ...
        if(backend_settings.debug_level)console.log('ping host: ' + host.HostName);
        ping.sys.probe(host.HostName, function(isAlive){
            if (isAlive == null) return;//skip phantom probe results
            host.isAlive = isAlive;
            if(backend_settings.debug_level)console.log(host.HostName + ' is alive ' + isAlive);
            ...
            if (isAlive){
               // try {
                    if(backend_settings.debug_level)console.log('trying to connect to host: ');
                    if(backend_settings.debug_level)console.log(host);
            ...
            if(backend_settings.debug_level)console.log('dump miner hosts: ');
            if(backend_settings.debug_level)console.log(hosts)
        });
    });
